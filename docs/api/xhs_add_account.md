...
| 字段 | 说明 |
|------|------|
| `sessionId` | 会话 ID，用于后续的 `xhs_check_login` 和 `xhs_submit_verification` |
...
2. 用户扫描二维码
   ↓
3. xhs_check_login(sessionId) → 检查状态：
...
返回二维码 URL 后，展示给用户扫描，然后调用 `xhs_check_login` 检查状态。
...
- 扫码后必须调用 `xhs_check_login` 完成登录流程
- 登录成功后会话自动持久化到数据库
