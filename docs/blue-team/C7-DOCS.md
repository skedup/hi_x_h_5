...
- [x] **登录表述**：修正 EN quick-start / account API 中「默认 headless 登录」；当前登录沿用 `config.browser.headless`（默认有头）
- [x] **环境变量表**：`CLAUDE.md` 补齐 `config.ts` 中 Wave A/B/C2 反检测开关（cooccurrence、xsec、quota、dedup、liveness、headlessWriteGate 及 B 系子项），不虚构未实现的 C1/C3/C8 env
- [x] **代码注释**：`auth.ts` 中 headless 说明改为「默认 false（有头）」
