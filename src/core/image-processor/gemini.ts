 ...
 * 分析多张图片
 */
export async function analyzeImages(imagePaths: string[], prompt: string): Promise<string> {
  const parts: any[] = [];
...
export function parseJsonResponse<T>(text: string): T {
  ...
  if (!jsonStr.startsWith('{') && !jsonStr.startsWith('[')) {
    ...
    const start =
      startBrace >= 0 && startBracket >= 0 ? Math.min(startBrace, startBracket) : Math.max(startBrace, startBracket);
    if (start >= 0) {
