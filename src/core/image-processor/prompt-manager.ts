 ...
 * 渲染 prompt 模板
 */
export async function renderPrompt(templateName: string, variables: Record<string, any>): Promise<string> {
 ...
 * Phase 2: 布局规划 prompt
 */
export async function getLayoutPrompt(analysis: any, style: string, palette: any, canvasSizes: any): Promise<string> {
 ...
 * Phase 5: 质量检查 prompt
 */
export async function getQualityPrompt(topic: string, slideCount: number): Promise<string> {
