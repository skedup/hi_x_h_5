...
| `XHS_MCP_HEADLESS` | `false` | 浏览器无头模式；默认有头。写操作在 `XHS_MCP_AD_HEADLESS_WRITE_GATE=true`（默认）且 headless 时被拒绝 |
...
- Login follows `config.browser.headless` (default headful); avoid `XHS_MCP_HEADLESS=true` for login fingerprint hygiene
